...
pub fn print(color: &str, output: &str) -> Result<()> {
    match to_color(color) {
        Color::Cyan => {
            let cyan =
                format!("{}", Style::new().cyan().bold().apply_to(output));
            Term::stdout().write_line(&cyan)?;
        }
        Color::Blue => {
            let blue =
                format!("{}", Style::new().blue().bold().apply_to(output));
            Term::stdout().write_line(&blue)?;
        }
        Color::Black => {
            let black =
                format!("{}", Style::new().black().bold().apply_to(output));
        ...
        Color::Green => {
            let green =
                format!("{}", Style::new().green().bold().apply_to(output));
            Term::stdout().write_line(&green)?;
        }
        Color::Yellow => {
            let yellow =
                format!("{}", Style::new().yellow().bold().apply_to(output));
            Term::stdout().write_line(&yellow)?;
        }
        Color::Magenta => {
            let magenta =
                format!("{}", Style::new().magenta().bold().apply_to(output));
            Term::stdout().write_line(&magenta)?;
        }
        Color::White => {
            let white =
                format!("{}", Style::new().white().bold().apply_to(output));
